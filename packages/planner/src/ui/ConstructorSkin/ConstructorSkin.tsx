 ...
 *
 * Ширины рейла здесь нет и быть не может: он стоит слева **снаружи** рамки холста (задача 0089) и ни одного
 * пикселя канваса не закрывает. Левая полоса берётся на всю высоту, хотя панель инструментов короче кадра:
 * insets — прямоугольная рамка, и запас здесь в пользу пользователя (геометрия гарантированно достижима мышью).
 * Правой полосы нет вовсе — колонка со сводкой и контролами целиком лежит внутри нижней. Верхней тоже нет:
 * шапка редактора живёт над холстом, а не поверх него.
 */
export const CONSTRUCTOR_SKIN_INSETS: Readonly<ViewportInsets> = Object.freeze({
  left: OVERLAY_GAP + TOOL_PANEL_WIDTH + OVERLAY_GAP,
  right: 0,
  top: 0,
...
/**
 * Оверлеи конструктора (задача 0061): панель инструментов, группа контролов холста, место под сводку и полоса
 * подсказки клавиш. Монтируется **детьми `<Planner />`** — то есть внутри `PlannerContext` и внутри **рамки
 * холста** (ADR 0020 P6): панели живут абсолютом поверх холста, холст ширину не теряет (handoff
 * `docs/ui/handoffs/planner/planner-editor-ui.md`, «Каркас»; решение 13 брифа). Отступы 12px здесь считаются от
 * краёв холста — то есть от рамки, а не от окна.
 *
 ...
 * - **рейла видов и снапа** — он стоит слева снаружи рамки холста и потому принадлежит контейнеру `<Planner />`
 *   (задача 0089); скин целиком снимается вместе с видом, а рейл виден во всех видах;
 * - **шапки редактора 48px** — она принадлежит платформе (задача 0088), а не пакету;
...
export const ConstructorSkin: React.FC<ConstructorSkinProps> = ({ children }) => {
   ...
   * (находка критика 0061: панель съедала левую полосу, а `fitToContent` считал по всему кадру и клал
   ...
   */
  useEffect(() => {
    // На чужом виде оверлеи конструктора сняты, а рейл холст не закрывает — полос нет вовсе.
    projections.canvas2d.setViewportInsets(isConstructor ? CONSTRUCTOR_SKIN_INSETS : NO_INSETS);
