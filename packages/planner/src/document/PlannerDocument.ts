...
/**
 * Пол — самостоятельный контур (Q12) с **хранимой** вложенностью и **хранимым** флагом потолка
 * (ADR 0016 B4, правка по месту 2026-08-20, задача 0069).
 *
 * - `kind`: `outer` — пол, `inner` — пол-вычитание (дырка). Вложенность обязана быть хранимой, а не выводимой
 *   из containment: спека 02 «Осиротевшие внутренние полы» требует **удалять** дырку, оставшуюся без хозяина —
 *   при выводимой вложенности она молча стала бы обычным полом, и пользователь увидел бы, что вырез зарос.
 * - `ceilingHidden`: «скрыть потолок этого пола». Спека 02 «Видимость потолка» хранит флаг **не на комнате,
 *   а на полу**, наследует его по перекрытию площади независимо от атрибутов комнаты, а при слиянии полов
 *   берёт от **первого пола-донора**. Дефолт — `false`.
 *
 * Больше на полу ничего не хранится: материалы — ADR J, профиль плинтуса и видимость крышки зоны — шаг 7.
 */
