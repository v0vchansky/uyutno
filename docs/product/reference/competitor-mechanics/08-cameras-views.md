...
**Формат 2 — сырой полный `getCameraData()`** (стр. 46751): `{type:"perspective", fov, pan, tilt, distance, position, anchor, rotation, clipping}` (всё в радианах/сырое). Через него `R2D.Viewers.saveCameraData()` (стр. 17075) снапшотит все три сцен-viewer'а разом — фасад только делегирует, сам `structuredClone` живёт в per-viewer `saveCameraData` (стр. 47054 3D / 47872 2D / 48621 walk); `getSavedCameraData` (стр. 17072) делегирует в `currentViewer`, `clearSavedCameraData` (стр. 17107) чистит view3d + viewWalk.
...
Пересчитывает `width = window.innerWidth − getOffsetWidthDueToOpenedPanels()`, `height = window.innerHeight`. Оффсет = `(dynamicPanel? 310/190 : 0) + (categoryTree? 310/190 : 0) + (controlPanel? 40 : 0)` (стр. 16843; `categoryTreePanelWidth = panelsWidth` — те же 310/190, стр. 16809). При `isRenderMakeActive` клампит в 16:9-леттербокс. Триггерится на resize окна (стр. 239). Панели (dynamic/categoryTree/control) влияют на макет только через этот оффсет; сам менеджер их DOM не рисует.
...
- **`toCenter()`** (стр. 46640): по `getSceneBox3()`; пустая сцена → anchor `(0,110,0)`; иначе `updateCameraViewPosition(center.x, 110, center.z, distance.max, π/4, π/4)` (**изометрия 45°/45° на макс-дистанции**) + `zoomToOptimal(box3)`. `findOptimalCamDist(w,h,d)` (стр. 37424–37441) фитит не бокс, а его **диагональ с удвоенным Y**: `diametr = sqrt(w² + (2h)² + d²)`, выбор vFov/hFov по сравнению `sceneAspect = w/(2h)` с aspect камеры, `dist = diametr / (2·tan(fov/2))` плюс эмпирический запас `+ depth·aspect·0.5`; затем `zoomToOptimal` (37262) нормализует dist в `[minDistance, maxDistance]` и **квантует** его до ближайшего из 41 значения таблицы `scaleValues` (`updateScalePointer` 36984 → `findIndexOfNearestScaleValue`, таблица 36961), после чего `setDistance`.
- **Экстра:** авто-вращение `startAutoRotation(speed=0.002)` (стр. 47150, `setInterval` 60fps, инкремент pan); `cameraJump(params)` (стр. 37443) — анимация anchor/distance/pan/tilt с `easeInOutCubic`, `animTime=1000мс`, 60fps, shortest-arc; режим «модель как anchor» (`modelAsAnchorParams {minDist:20,maxDist:500,step:20}`, объявлены в ctor `Renderer3DPerspective` ~стр. 36955, мутируются `updateModelAsAnchorParams`) с гардом `camY>0`.
...
- **Скайбокс** (`R2D.Scene`, стр. 14461): `THREE.SphereGeometry(6000, 60, 40)` + `.scale(-1,-1,1)` (нормали внутрь), `Mesh`. `MeshBasicMaterial({color:'#ffffff'})` (14463) — только у пустой сферы-заглушки; загруженное небо получает `skyboxMesh.material = matViewer.getMaterial()` из `PoolMaterials.create(id,'','pano')` → `ObjectViewer3DMaterial` (Phong-плейсхолдер `MeshPhongMaterial({color:0x999999}, DoubleSide)` 32133–32137, после загрузки — материал из GLTF, `material = obj.material` 32155). Следует за камерой: `repositionSkybox()` = `skyboxMesh.position.set(cameraPos)` (стр. 14480) — читается как бесконечный фон; в walk вызывается на каждый move/zoom (48184–48201), в orbit-3D — только в render-make ветке `if (isRenderActive)` (46545–46549, 46594–46596). Эквиректангулярная проекция достигается **геометрически** (камера внутри вывернутой сферы), а не через `texture.mapping`.
...
- **Персистенс**: `scene.skybox = {id, r, tags}` (стр. 13081, 13911), id пушится в construction resource list; restore `setSkyboxMat(id)` + `setSkyboxRotation(r)` (стр. 12825). Дефолт `getDefaultPanoId()` (стр. 14766). `checkSkybox` (стр. 14588–14645) тогглит видимость по типу viewer/render-make: скайбокс показывается **только в walk и в render-make режимах** (topView — `showSkybox` кроме `'transparent'`; exterior/interior/walk — `showSkybox`, но при дефолтном pano (`isSkyboxDefault`) — `hideSkybox` + terrain); в обычном 3D и в 2D — `hideSkybox()` + `setTerrainDefault()` (14642–14644). `R2D.panorams` (стр. 82) заполняется извне через `planner.setPanorams` (стр. 670).
- **«Точки захвата панорамы» = render-make пресеты, а не тур-граф**: `renderMakeType ∈ {'exterior','interior','topView','walk'}` (на стр. 46191 — лишь объявление `let renderMakeType = ''`; сам набор значений — инференс из ключей `savedPanoData` и гардов, правдоподобен). Каждый вид хранит `savedPanoData[renderMakeType] = {id, rotation, previewImgSrc}` (стр. 46791–46799); interior — отдельно в walk (`savedInteriorPanoData`, стр. 48355). Бандлятся с сохранённой камерой: `saveExteriorData(cameraData, panoData)` (стр. 46773), `saveTopViewData`, `saveInteriorData` (стр. 48353). Фасад: `Viewers.savePanoData`/`getSavedPanoData`/`clearSavedPanoData` (стр. 17097). То есть «360-вид» = выбор скайбокса под режим захвата; сама эквирект-картинка — **готовый ассет из каталога**; экспорт 360 полагался бы на `makeScreenShot` по точке, не на in-browser стичинг.
...
- **Множитель `height*2`** есть только в `findOptimalCamDist` (37424–37441: `sceneAspect = w/(2h)`, `diametr` с `(2h)²`, плюс `depth·aspect·0.5`) — недокументированная эвристика «воздуха». В `setCameraFromTopToBottom` (46327–46333) его **нет**: там `maxSide = max(bounds.width, bounds.height) + 80` по 2D-`scene.getBounds()` и `distance = maxSide / (2·tan(fov/2))`.
...
**Высокая уверенность** (прямая проверка исходника + перекрёстная сверка): архитектура «один общий рендерер + одна сцена, 4 камеры-обёртки»; индексы и порядок регистрации viewer'ов; логика `enableViewer`; трёхслойный `clearDepth`-пайплайн (bottom/middle/(contours)/view2DObjects/selection/top); `autoClear=false`; орбита-математика; **все числа в таблице** (fov/near/far/eye-height/distance/tilt/зум-таблицы/frame-counts 5 vs 2); render-on-demand логика; сериализация camera-state (компакт `{px,py,pz,tilt°,pan°,zoom0..1}` + сырой `getCameraData`; 3D-версия `getViewStateData` проверена построчно — стр. 46664–46678, отдельное замыкание от 2D-версии на стр. 47672); скайбокс как вывернутая сфера r=6000 с pano-материалом из каталога (`&category_tag=skybox`).

**Пробелы / чего нет в core-файле:**

1. **`URL_CAMERA_VIEWS` не существует** — есть только event-имя `UPDATE_CAMERA_VIEWS="updateCameraViews"` (стр. 857) и per-render-mode saved-camera'ы (exterior/interior/topView/walk), но это одиночные камеры для восстановления render-сетапа, **не тур-граф и не URL-пресеты**. Рядом в API объявлена и константа `planner.scene.UPDATE_TOURS` (стр. 859) — т.е. туры как концепт существуют, но вся их логика живёт во внешнем бандле. URL-round-trip (если есть) — тоже вне этого файла.
...
8. **Множитель `height*2`** в фит-математике — только в `findOptimalCamDist` (37424), не в `setCameraFromTopToBottom` (46327); это эмпирика без документированного обоснования (см. §выше).

**Чего не хватает для реализации:**

1. **Маппинг на современный THREE**: свой OrbitState `{anchor, pan, tilt, distance}` → `position`/`quaternion`; `outputColorSpace`/`toneMapping` выбирать самим (у конкурента — пробел №6 выше).
2. **Фильтры мульти-пасса** (BGFilter/ContoursFilter/OutlineFilter/SSAO) не разобраны — свой аналог на `EffectComposer`; сам паттерн `autoClear=false` + `clearDepth` и `RenderUpdater` (бюджет N кадров, `onProgress`/`onFinish`) переносятся дословно.
3. **Карта жестов не сведена** (MouseController FSM; тач: 1 палец 2D=move / 3D=rotate, 2 пальца = rotate/pan). Для нас: Pointer Events + `setPointerCapture`, пинч реализовать по-настоящему (у конкурента — dead code, пробел №4).
4. **Коллизии walk — с нуля** (у конкурента нет, §5a): raycast по направлению шага + радиус капсулы; паттерн click-to-jump (рейкаст, 80% пути, кап 500) взять как есть.
5. **Плавный переход 2D↔3D** — референса нет (свитч мгновенный); твин tilt π/2→1.25 + матч ортоширины↔дистанции проектировать самим.
6. **Сериализация камер**: компакт-формат готов, но 2D-зум сырой (−19…0.9), 3D — нормализованный 0..1 — унифицировать; конфликт `state:"2d"` двух вьюверов — их баг, не повторять.
7. **Resize/aspect-политика** не расписана (хардкод 800/600 при создании камер).
8. **360/туры** — референса нет (внешние бандлы); скайбокс переносим, но в r185 проще `scene.background` + `EquirectangularReflectionMapping`.
