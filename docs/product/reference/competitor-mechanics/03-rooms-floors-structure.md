...
- **`StateSelectedCover`** (L66882): multi-select of covers. Hover finds nearest cover point (within `WC.POINT_SIZE/scale`) or a cover side (`getSideByPoint`); can drag whole cover, drag a point, or **split a side** by inserting a new cover point mid-edge (`splitCurrentSide`, L66936; hover handle at `sideCenter`, grabbed within `WC.CENTER_SIZE/scale`, L67051-67052). This is a **separate implementation** of the same gesture the wall editor has — the wall one (`WC.StateEditing.splitCurrentSide`, L64742) also splits walls, areas and child covers, and lives in the idle hub rather than requiring a prior selection; see [01-walls.md → «Ручка деления стороны»](./01-walls.md). Zones (`CArea`) have no such handle at all. Exposes `setCeilingVisible`/`setCeilingHeight` applied to all selected covers. On `stop` re-runs `connectAllPoints`.
