...
- **Storage:** `localStorage` key `r2d_clipboard` suffixed with the user key → `r2d_clipboard_<userKey>` (`R2D.Storage`, `17776`). Also a **global OS-clipboard bridge** `checkGlobalClipboard()` (`69405`) that reads `navigator.clipboard.readText()`; a payload `{action:"copy_to_clipboard", model}` is re-stored locally as `{type:"model", value: model}`.
