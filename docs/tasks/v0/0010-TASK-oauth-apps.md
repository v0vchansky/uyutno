...
- PR: — (внешняя регистрация, кода нет)
...
- [x] Приложение Yandex ID зарегистрировано, redirect URI (prod + dev) добавлены, креды выданы.
- [x] Креды сохранены в prod-`process.env`, инструкция по локальному запуску — в `apps/platform/README.md`.
- [ ] Официальный SVG-иконки Yandex ID добавлен в проект и готов к использованию в 0015.
- [x] URL-эндпоинтов и особенности провайдера зафиксированы в спеке фичи (`docs/product/features/auth.md`) или в комментарии к задаче.

## Заметки

- Официальный SVG Yandex ID пока не добавлен — в интерфейсе 0015 остаётся монограмма «Я» на фирменном фоне `#FC3F1D` из макета. Замена на официальный ассет — косметический follow-up, отдельным маленьким PR; на функциональность OAuth-flow не влияет.
