...
- Спека: —
- PR: 1eedff8 (прямой коммит в main)
...
- [x] Залогиненный пользователь на `/` видит в шапке только кнопку «В личный кабинет» (ведёт на `/projects`); «Войти» отсутствует.
- [x] Анонимный пользователь на `/` видит «Войти» + «Создать проект» — как раньше.
- [x] SPA-переход на `/login` или `/register` для залогиненного (например, из адресной строки через `navigate()` или ручным вводом с уже открытой вкладки) немедленно редиректит на `/projects`, форма логина не отображается.
- [x] Для анонима `/login` и `/register` продолжают открываться нормально.
- [x] Серверный `redirectIfAuthenticated` не тронут — hard-load `/login` под сессией по-прежнему редиректит на сервере.
- [x] Визуальная проверка через Playwright MCP: 1440 / 768 / 390; сценарии — аноним/залогинен на `/`, залогиненный переход на `/login`.
