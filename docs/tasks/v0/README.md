...
- **Мета для страниц.** Любая задача, заводящая новую страницу (`apps/platform/src/client/**/pages/**/*Page.tsx`), в приёмке обязана проверить наличие своих `<title>` и `<meta name="description">` у этой страницы (тексты осмысленные, не дубли соседних страниц). Детали правила — в [`apps/platform/src/client/CLAUDE.md`](../../../apps/platform/src/client/CLAUDE.md), раздел «Страницы (`pages/`)»; здесь — только напоминание, чтобы требование было видно на уровне доски.
...
| 0029 | TASK | [Выход из аккаунта: `/auth/logout` + клиент](0029-TASK-logout.md)                                                          | [x]    | 0011, 0026       | — (прямой коммит в main)       |
...
| 0019 | TASK | [404-страница для неизвестных путей](0019-TASK-not-found-page.md)                        | [x]    | 0004       | —   |

### 0025 · Обновление макетов (layout / лендинг / авторизация) · [x]

Спека и общее описание: [0025-EPIC-layout-landing-auth-refresh.md](0025-EPIC-layout-landing-auth-refresh.md).

| #    | Тип  | Название                                                                                        | Статус | Зависит от | PR                       |
| ---- | ---- | ----------------------------------------------------------------------------------------------- | ------ | ---------- | ------------------------ |
| 0026 | TASK | [PublicLayout: 4 состояния шапки + меню профиля + бургер + подвалы](0026-TASK-public-layout.md) | [x]    | 0004, 0023 | — (прямой коммит в main) |
| 0027 | TASK | [Сверка лендинга с новыми макетами](0027-TASK-landing-sync.md)                                  | [x]    | 0026       | — (прямой коммит в main) |
| 0028 | TASK | [Сверка `/login` и `/register` с новыми макетами](0028-TASK-auth-screens-sync.md)               | [x]    | 0026       | — (прямой коммит в main) |

### 0035 · Экран `/projects` (список проектов) · [x]

Спека и общее описание: [0035-EPIC-projects-screen.md](0035-EPIC-projects-screen.md).
...
| #    | Тип  | Название                                                                                                                                                                                    | Статус | Зависит от                   | PR                       |
| ---- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------- | ------------------------ |
| 0051 | TASK | [ADR C: геометрический пайплайн планера (контуры → лента/митринг → триангуляция → комнаты → оси)](0051-TASK-planner-adr-geometry-pipeline.md)                                               | [x]    | 0043                         | — (прямой коммит в main) |
| 0052 | TASK | [ADR D + E: мутации/транзакции и undo; автомат инструментов, пайплайн ввода/снапа и Canvas2D-конструктор](0052-TASK-planner-adr-transactions-undo-and-tools-input.md)                       | [x]    | 0043 (принятие — после 0051) | — (прямой коммит в main) |
| 0053 | TASK | [Геометрическое ядро 1/2: предикаты и примитивы, лента стены и митринг, валидация контура](0053-TASK-planner-geometry-predicates-and-wall-band.md)                                          | [x]    | 0051                         | — (прямой коммит в main) |
| 0054 | TASK | [Геометрическое ядро 2/2: обвязка триангуляции, детекция комнат, оси стен, реальные `normalize`/`rebuild` → `DerivedState`](0054-TASK-planner-geometry-triangulation-rooms-axes-rebuild.md) | [x]    | 0053                         | — (прямой коммит в main) |
| 0055 | TASK | [Команды планировки как транзакции + реальный `history` (снапшоты, два контейнера, coalescing, лимит 100, dirty-флаг)](0055-TASK-planner-layout-commands-and-history.md)                    | [x]    | 0054, 0052                   | — (прямой коммит в main) |
| 0062 | TASK | [Снап: чистые функции (лесенка приоритетов, гайды) и индекс кандидатов](0062-TASK-planner-snap-functions-and-candidate-index.md)                                                            | [x]    | 0053, 0052                   | — (прямой коммит в main) |
| 0057 | TASK | [Автомат инструментов (`tools`), контракт ввода + инструмент «Стены»](0057-TASK-planner-tools-fsm-and-walls-tool.md)                                                                        | [x]    | 0055, 0062                   | — (прямой коммит в main) |
| 0058 | TASK | [Инструменты «Прямоугольная комната» и «Комната по точкам»](0058-TASK-planner-tools-rect-and-polyline-room.md)                                                                              | [x]    | 0057                         | — (прямой коммит в main) |
...
| 0056 | TASK | [Canvas2D-вьювер конструктора: отрисовка документа и производного, DOM-ввод, зум/пан, render-on-demand, resize/dispose](0056-TASK-planner-canvas2d-constructor-viewer.md)                   | [x]    | 0054, 0052, 0057             | — (прямой коммит в main) |
...
| 0082 | TASK | [Ручной Save и серверный автосейв раз в 60 с](0082-TASK-planner-manual-save-and-server-autosave.md)                                             | [x]    | 0081             | —   |
| 0083 | TASK | [Локальный черновик демо-роута: `planner_demo_draft`, 30 с, diff-guard, квота](0083-TASK-planner-demo-local-draft.md)                           | [x]    | 0081             | —   |
...
| 0076 | TASK | [`errorMiddleware` не знает ошибок Express: 413 приходит клиенту как 500](0076-TASK-error-middleware-express-errors.md)               | [x]    | 0008                     | —                              |
