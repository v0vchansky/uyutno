...
- [ ] Страница `/login` рендерится и визуально совпадает с макетом на desktop / tablet / mobile.
- [ ] `POST /api/v1/auth/login` с валидной парой создаёт сессию, устанавливает cookie, клиент редиректит на `/projects` (или на `from`).
- [ ] Неверная пара логин-пароль возвращает 401; плашка над кнопкой; курсор в поле email (или без — на усмотрение, лишь бы форма редактируема).
- [ ] Rate limit срабатывает на 6-й попытке (5+1) в 15-минутном окне, ответ 429; UI показывает нейтральную плашку.
- [ ] Enter в любом поле отправляет форму. Клавиша Tab — обычный tab-порядок.
- [ ] Уже залогиненный пользователь редиректится с `/login` на `/projects` (SSR-редирект из 0011).
- [ ] `?from` сохраняется в ссылке «Зарегистрироваться» и «Забыли пароль?».
- [ ] После успешного входа с `?from=/projects/abc` пользователь оказывается на `/projects/abc`; с невалидным `from` — на `/projects`.
- [ ] Страница рендерит свои `<title>` и `<meta name="description">`.
