...
interface ProjectOpenInput {
  /**
   * Фаза 1 — карточка проекта (`GET …/:id`): имя для шапки и он же признак «проект существует и он ваш».
   * `project: undefined` при снятом `isPending` означает **404 от сервера** про этот id, а не «не нашёлся
   * в списке галереи», как было до задачи 0095.
   */
 ...
 * 3. сервер ответил на карточку 404 — 404-страница;
